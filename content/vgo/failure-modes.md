...
This post relies heavily on basic concepts from game theory. Earlier drafts did not, because I was concerned it might be too off-putting, in addition to my own misgivings about game theory as a methodology. But, without a framework for more precisely expressing the problems inherent in MVS, the concerns came off as fuzzy. And the truth is, dependency management is a bit of a dream problem for game theory, particularly [evolutionary game theory](https://en.wikipedia.org/wiki/Evolutionary_game_theory): most relevant actions are part of an immutable public record, decision points can be readily identified, causality can be traced, identity is stable, and costs are easily identified.

That said, this post is not a formal game theoretic argument. (If there are any academics out there who'd like to collaborate on that, please [reach out](https://twitter.com/sdboyer)!). For example, I've omitted discussion of payoffs - a necessary part of a proper model, but tricky in part because the motivation for participation in FLOSS is, at the very least, the subject of some debate.
...
As we've seen here, MVS has the very same kind of problem. What it adds, however, is the reverse structure: "responsibility without authority." That's the bit where, when the ecosystem is in a degraded mode with respect to compatibility, it's your job to work to fix other peoples' problems, where "problem" is often "they didn't do enough work to keep up with everyone else." And we should expect the ecosystem to always be in such a degraded mode, according to Russ' own statements[^3], game theory, and the adage that [complex systems always run in degraded mode](https://blog.acolyer.org/2016/02/10/how-complex-systems-fail/).

In other words: "Welcome to Go! Now, shave this yak somebody left lying around."

This is why I find the notion that [only code changes are a permanent solution to incompatibility](https://github.com/golang/go/issues/24301#issuecomment-390766926) to be fundamentally uninteresting. Obviously it's true. And we'll get there - but, eventually, and without the tool biting us. We're a community, not a pool of interchangeable workers operated by a distributed scheduler.
...
[^2]: It's "indirect reciprocity" because Aparna's benefits derive from the environment - a compatible ecosystem - rather than any one single other actor. Module graphs don't fulfill the requirements for [network reciprocity](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3279745/pdf/nihms49939.pdf) because, while cycles are possible in the module graph, they are overwhelmingly directed in practice.

[^3]: From [the MVS blog post](https://research.swtch.com/vgo-mvs#upgrade-speed): _"But I think in practice dependencies will move forward at just the right speed, which ends up being just the right amount slower than Cargo and friends."_ 'Just the right speed' is necessarily slower than 'everything updated all the time,' which means the ecosystem is in a degraded mode.
